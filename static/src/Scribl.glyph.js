...
var Glyph = Class.extend({
    ...
    */
	init: function(type, pos, length, strand, opts) {
      ...
      glyph.addDrawHook(function(theGlyph) {
        if (theGlyph.ntLevel != undefined && theGlyph.seq && theGlyph.lane.chart.convertPixelstoNts() < theGlyph.ntLevel){
    ...
    */
	getPixelLength: function() { 
      var glyph = this;
      return ( glyph.lane.chart.convertNtsToPixels(glyph.length) || 1 );
   },
    ...
    * @return {Integer} positionX - in pixels
    * @api public        
    */	
   getPixelPositionX: function() { 
      ...
      return Math.round( glyph.lane.track.chart.convertNtsToPixels( position ) + offset);
   },
