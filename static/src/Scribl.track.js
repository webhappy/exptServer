...
var Track = Class.extend({
    ...
    */
	init: function(chart) {
      ...
      this.chart = chart
      this.lanes = [];
      this.addLane(); //HACK! Force everything to one lane wtf?
      this.ctx = chart.ctx;
...
/*      for (var j=0; j < this.lanes.length; j++) {
         ...
         var spacer = 3/this.chart.convertNtsToPixels() || 3;
      ...
      new_lane=false; //force everything to one lane!
        curr_lane = this.lanes[0];
